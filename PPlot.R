    ...
    lx = 0.8, ly = 4, catname = NULL, ps = FALSE, file = "./prefplot.eps") 
{
    ...
    if (ind == TRUE) {
        for (i in 1:nInd) {
            for (j in 1:nCat) {
                lines(density(prefres$IndPref[i, j, (burn + 1):mcmc]), 
    ...
    if (pop == TRUE) {
        for (j in 1:nCat) {
            lines(density(prefres$PopPref[j, (burn + 1):mcmc], 
                adj = dadj), col = "black", lty = 1, lwd = 3)
        }
    }
    if (dist == TRUE) {
        alphas <- numeric(nCat)
        for (j in 1:nCat) {
            alphas[j] <- median(prefres$PopPref[j, (burn + 1):mcmc]) * 
                median(prefres$PopVar[(burn + 1):mcmc])
        }
        x <- rdirichlet(10000, alphas)
        for (j in 1:nCat) {
            lines(density(x[, j]), col = colors[j], lty = 1, 
                lwd = 4)
        }
    }
    if (leg == TRUE) {
        ...
        if (ind == TRUE) {
            legend(lx, ly, catname2, col = c(colors[1:nCat], 
                colors[1:nCat]), lty = c(rep(1, nCat), rep(2, 
                nCat)), lwd = c(rep(2, nCat), rep(1, nCat)))
        }
        if (ind == FALSE) {
            legend(lx, ly, catname2[1:nCat], col = c(colors[1:nCat], 
                colors[1:nCat]), lty = c(rep(1, nCat), rep(2, 
                nCat)), lwd = c(rep(2, nCat)))
        }
    }
    if (ps == TRUE) {
        dev.off()
